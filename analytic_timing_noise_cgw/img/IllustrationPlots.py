...
ax1.step(time, DeltaF1, lw=2, color="b", where="post")
...
ax1.set_xlabel("Time")
ax1.set_xticks([end+kappa*dt])
ax1.set_xticklabels(["$t_{i}$"])

ax1.set_yticklabels([])
ax1.set_ylabel("$\Delta\dot{f}$", rotation="horizontal", 
              labelpad=15.0)
r = Rectangle((0, 0), 1, 1, color="g", alpha=.5) # creates rectangle patch for legend use.
plt.legend([p1, r], [r"$t_{\textrm{ref}}$", 
