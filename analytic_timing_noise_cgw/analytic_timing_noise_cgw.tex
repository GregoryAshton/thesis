...
This issue was investigated by \citet{watts2008} who quantified the effect using
...
They then estimated a worst-case decoherence time by using the maximal
...
In the previous two sections, we have used the random walk model of timing,
given in Section~\ref{sec: defining a random walk} which may appear
non-physical: the random walk `jumps' every $\dT$, but what
physics determines this length?  In this section, we will show that such a
...
(see for example \citet{weiss2006course}) that $\Delta X_i(\dT)$ is normally
distributed. To mean can be calculated using Wald's
Eqn.~\citep{wald1944cumulative} which gives expectation and 
\begin{align}
...
because $\Delta X_{i}(\dT)$ and $\Delta X_{i-1}(\dT)$ are normally distributed
random variables and therefore it can be shown (see for example
\citeauthor{wolframdifference}) that $\tn X_i (\dT)$ is also normally
