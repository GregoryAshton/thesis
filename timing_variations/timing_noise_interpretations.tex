...
\begin{align}
    ...
    = \frac{1}{6}S_{\mathrm{SN}}T^{3} && \textrm{(spin-down).}
\end{align}
...
periods longer than the observation. This effect is evidence from the Crab
pulsar which was found by \citet{Boynton1972} to be consistent with
frequency-like noise, but in Fig.~\ref{fig: timing noise example} we clearly
...
any non-spherical body for which the angular momentum is not aligned with a
...
\begin{equation}
    I = \left[\begin{array}{ccc}
            ...
            \end{array}\right],
\end{equation}
where $I_0$ is the total moment of inertia of the star involved in precessin
and $\Delta I/I_0 \ll 1$ is the measure of oblateness or prolateness. If the
...
\end{split}
\end{align}
where $\epsilon = \Delta I / I_0$,
$a_0$ is the angle between the spin-vector and the body frame $z$ axis, and
...
\end{align}
where $I_\textrm{psf}$ is the fraction of superfluid pinned to the crust. To
explain the magnitude of observed glitches this second term was found to be
$\sim 0.01$. Inserting this into Eqn.~\eqref{eqn: tauP free}, we see that the
free precession period must be approximately 100 times the spin-period which is
signicantly longer than observed precession periods.
...
remains an open question. Resolving this, and determining exactly where the
...
authors measured the spin-down rate of each pulsar (for a description of the
so called observational-method used to do this see Sec.~\ref{sec: switching}) over a $\sim20$~year
period; they found a variety of smooth periodic fluctuations with typical
periods of years. In Fig.~\ref{fig: lyne 2010 fig2} we reproduce their original
plot showing the periodic variations in spin-down rates.
\begin{figure}
    \centering
...
context of free precession in Sec.~\ref{sec: free precesson}.
...
magnetosphere periodically and suddenly between two magnetospheric states which
...
This is a purely deterministic model and, having generated the
spin-down values, we can integrate twice to get the phase. From the phase, we
can use the method of \citet{Lyne2010} (described in Sec.~\ref{sec: switching})
...
is missing a substantive physical explanation.
...
pulsars used in \citet{Lyne2010} as evidence of magnetospheric switching. Since
...
To test the hypothesis that an observed $\dot{\nu}$ variations calculated using
the observer-method is due to the \citet{Perera2014} model of switching, the
$\dot{\nu}$ calculation can be repeated at two or more time averaging
baselines.  Variations in the ratio of the two peaks would provide clear
evidence in favour of this model. Of course in this example we have considered
a simple situation where we have known apriori which peak is being
underestimated due to the observational-method of calculating the time averaged
spin-down rate. In real data, the situation may be more complicated: it could
be that it is the trough which is overestimated instead of the peak being
underestimated; or more than one switching duration may be shorter than the
time averaging baseline.  Nevertheless, the changing ratio of the two peaks (or
troughs) may still prove a simple way to test the Perera model.
