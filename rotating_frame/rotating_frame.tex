...
to an inclined magnetic dipole \citep{Pacini1968, Gunn1969}. In Sec.~\ref{sec:
...
the anomalous component (defined in Sec.~\ref{sec: defining the model}). The
...
precession': in Sec.~\ref{sec: biaxial NS with anomalous torque} we will
...
\meta{ The rest of this chapter is organised as follows: in Sec.~\ref{sec:
defining the model} we will define the Euler rigid-body equations and the
\citet{Deutsch1955} torque. In Sec.~\ref{sec: spherical} we will discuss the
simple case of a spherical star, then in Sec.~\ref{sec: biaxial NS with no anomalous
torque} and Sec.~\ref{sec: biaxial NS with anomalous torque} we will discuss a
...
in Sec.~\ref{sec: conclusion rotating frame}.}
...
end states are then the (a) and (c) pictures in Fig.~\ref{fig: sphere ellipsoid}.

If they both begin from the same initial configuration, then one of them must pass through
the special $J^{2}=2EI_{0}$ state corresponding to Fig.~\ref{fig: sphere ellipsoid}(b).
...
($\phi$ monotonically increasing) corresponding to Fig.~\ref{fig: sphere ellipsoid}(c).
In Fig.~\ref{fig: NS A}(b), the discontinuity is in fact exactly the point
...
the~$\1$~axis. In Fig.~\ref{fig: NS A 3D}(b) we also plot the solution in
...
body frame axis. Fig.~(a) shows three distinct time steps; blue, the time before the
discontinuity, red is during the discontinuity, and black is some time after.
Fig.~(b) demonstrates the solution during the discontinuity, the changing colour is
...
\meta{
Transforming to the effective body frame in Fig.~\ref{fig: NS B}(b), we find
...
In Fig.~\ref{fig: NS C}, for all values of $\chi$, the spin-vector is found
...
%Fig.~\ref{fig: phase space}. We therefore present a more realistic phase
