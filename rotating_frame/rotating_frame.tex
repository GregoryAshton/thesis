...
neutron star spherical}. This shows a brief spin-down which halts once the spin-vector
aligns with the magnetic dipole $a \rightarrow \chi=30^{\circ}$.  The azimuthal
angle $\varphi$ does not vary; this is expected since precession, a monotonic
increase in $\varphi$, does not occur for spherical bodies. This result agrees
with the analytic calculations: the alignment occurs on a few times the
spin-down time scale $\tauS = 9\times 10^{6}$~s.
\begin{figure}[ht]
\centering
	\centering
	\includegraphics[width=0.5\textwidth]
     {{Spherical_Plot_one-component-model_eta_0.00e+00_chi0_3.0000000000e+01_omega0_1.00e+04_epsI3_0.00e+00_epsA_5.00e-11_a0_5.0000000000e+01_T_1.00e+08_n_1000_epsI1_0.00e+00_AnomTorque_0}.png}
\caption{Plot of the spherical coordinates of the spin-vector for a
spherical star. Note that the magnetic dipole is inclined at $\chi=30^{\circ}$
to the $\z$ axis.}
\label{fig: neutron star spherical}
\end{figure}

\section{Biaxial neutron star with no anomalous torque}
\label{sec: biaxial neutron star with no anomalous torque}
...
Star &$\epsilon_{\mathrm{I}}$ & $\epsilon_{\mathrm{A}} $ & $B_{0} $[Gauss] & $\tau_{\mathrm{S}}$ [s] & $ \tau_{\mathrm{P}}$  [s] \\ \hline
A & $ {1.0}{\times} 10^{-9} $ & $ {5.0}{\times} 10^{-11} $ & $ {1.3}{\times} 10^{13} $ & $ {9.0}{\times} 10^{6} $ & $ {9.8}{\times} 10^{5} $\\
B & $ {4.0}{\times} 10^{-11} $ & $ {5.0}{\times} 10^{-11} $ & $ {1.3}{\times} 10^{13} $ & $ {9.0}{\times} 10^{6} $ & $ {2.4}{\times} 10^{7} $\\
C & $ {1.0}{\times} 10^{-15} $ & $ {5.0}{\times} 10^{-11} $ & $ {1.3}{\times} 10^{13} $ & $ {9.0}{\times} 10^{6} $ & $ {9.8}{\times} 10^{11} $\\ \hline
\end{tabular}
\caption{Table of $\epsI$ and $\epsA$ and the inferred magnetic field and
    initial timescales for the three simulated stars. In calculating the
    magnetic fields and timescales, we have used a canonical value of
    $R=10^{6}$~cm for the radius, the initial value
    of the spin frequency, $\omega_{0} = 10^{4}$ rads/s and the initial
    value of $a_0=50^{\circ}$. As $\omega$ and $a$ evolve, the timescales of
    each star will also evolve from these initial values.}
...
\begin{figure}[ht]
\centering
\includegraphics[]{{phase_space_no_anom}.png}
...
% {{Spherical_Plot_no_anom_chi_30.0_epsI_1.0e-15_epsA_5.0e-11_omega0_1.0e4_t1_1e8}.png}}&
    \subfloat[$\chi=75^{\circ}>\chi_{\textrm{cr}}$]{\includegraphics[width=0.48\textwidth]
...
% {{Spherical_Plot_no_anom_chi_75.0_epsI_1.0e-15_epsA_5.0e-11_omega0_1.0e4_t1_1e8}.png}}
\end{tabular}
...
anomalous torque contribution was neglected in Section~\ref{sec: biaxial neutron star
...
anomalous torque timescale as given in Eqn.~\eqref{eqn: anomalous timescale}. Three
...
\begin{figure}[ht]
\centering
\includegraphics[]{{phase_space}.png}
\caption{Phase space diagram including the anomalous torque contributions. This
         differs from Figure~\ref{fig: phase space no anom} in that there is an
         additional grey band marking `region B' where $\tauS>\tauP>\tauA$.}
\label{fig: phase space}
\end{figure}

\subsection{Neutron star A in region \texorpdfstring{$\tauS>\tauA>\tauP$}{}}
\label{sec: A}

We expect the alignment of the spin axis to agree with the findings in
Section~\ref{sec: A_NA} and study the changes in behaviour due to the
introduction of the anomalous torque. Working in the effective body frame
rotates the solutions (for star A) by a few degrees as shown in Table~\ref{tab:
A B C params beta}.  Plotted in Figure~\ref{fig: neutron star A} are the
spherical coordinates of the spin-vector in the effective body. This can be
compared with the result in Figure~\ref{fig: neutron star A_NA}, which is the
same system without the anomalous torque component.
...
% {{Spherical_Plot_Transform_chi_30.0_epsI_1.0e-9_epsA_5.0e-11_omega0_1.0e4_eta_1.0e-4}.png}} &
    \subfloat[$\chi=75^{\circ}>\chi_{\textrm{cr}}$]{\includegraphics[width=0.48\textwidth]
...
%{{Spherical_Plot_Transform_chi_75.0_epsI_1.0e-9_epsA_5.0e-11_omega0_1.0e4_eta_1.0e-4}.png}}
\end{tabular}
...
\centering
\begin{tabular}{cc}
    \subfloat[]{\includegraphics[trim = 30mm 30mm 30mm 30mm, clip=true , width=0.48\textwidth]{{Angle_Space_Plot_3D_chi_75.0_epsI_1.0e-9_epsA_5.0e-11_omega0_1.0e4_eta_1.0e-4}.png}} &
    \subfloat[]{\includegraphics[trim = 30mm 30mm 30mm 30mm, clip=true , width=0.48\textwidth]{{ThreeD_Plot_Cartesian_chi_75.0_epsI_1.0e-9_epsA_5.0e-11_omega0_1.0e4_eta_1.0e-4}.png}}
\end{tabular}
...
%\section{Connection with the known neutron star population}
%\label{sec: Connection with the known neutron star population}
%TBD

%\label{sec: connection with pp}
%
%Having an understanding of how the different types of solutions behave, we now
%compare this with the known neutron star population to estimate which types of solutions
...
%\citet{Lyne2010}. The 18 neutron stars where listed along with their: spin frequency,
...
%deformation may be calculated by rearranging Eqn.~\eqref{fig: free
%precession}:
%\begin{equation}
%\epsI = \frac{1}{\nu F}.
%\end{equation}
%We use this as a first approximation to discuss the types of solution that may
%be relevant to the neutron star population, but recognise that we have yet to account
...
%    the neutron stars from \citet{Lyne2010} sit assuming the periodic oscillations
%are due to precession.}
%\label{fig: phase space for actual spin frequency}
%\end{figure}
%Plotting the results in Figure~\ref{fig: phase space for actual spin frequency} we observe that all neutron stars exist in Region A of the phase diagram.
%
%
...
%behaviour in several neutron stars, they predict that the governing equations should
...
%of the neutron star will damp the precession. We aim to reproduce this result by
