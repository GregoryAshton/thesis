...
The rotation of a neutron star is gradually slowed by torques. For most
isolated stars, the dominant torque is thought to be the electromagnetic
dipole. In Sec.~\ref{sec: free precession} we introduced the free precession,
in this section, we will study precession under the action of a dipole torque.
The form of this torque was first written down by \citet{Deutsch1955} where it
was found there there were two distinct components: the spin-down component and
the anomalous component (defined in Sec.~\ref{sec: defining the model}). Often
this second component is neglected and in doing so it is possible to find
analytic solutions; for example, \citet{Goldreich1970} uses these to discuss
the alignment of the magnetic axis. The addition of the anomalous torque makes
analytic solutions difficult, so \citet{Melatos2000} found numerical solutions
for the full torque. In doing so, the authors discovered solutions with `persistent
precession', something which is not yet understood in the literature.

In this chapter, we will solve the Euler rigid-body equations numerically and
compare with known analytic and numerical solutions. This will
familiarse the reader with current work in the field and achieve two tasks:
first, to understand the role of the anomalous torque with respect to
precession (and in doing so clear up confusion about persistent precession);
second, to provide an intuitive understanding of the alignment predictions made by
\citet{Goldreich1970}
...
solutions can be found as we demonstrated in Sec.~\ref{sec: free precession}.
...
The rest of this chapter is organised as follows: in Sec.~\ref{sec: spherical}
...
vector having an inclination angle of $a_0=50^{\circ}$ and azimuth of $\phi=0^{\circ}$.
...
proportional up to factors of $\epsI$. The evolution of the spin
...
find the critical value $\chi_{\textrm{\textrm{cr}}}\approx55^\circ$.  This is in agreement with
the findings of \citet{Goldreich1970}.  Moreover, by calculating the condition
in this way, we have found that the critical value corresponds to a tipping
...
scales means the precession, the alignment of the spin-vector, and the
spin-down all occur on the same timescale.
...
For the three NSs (A, B, and C as first used in Sec.~\ref{sec: biaxial NS with no anomalous torque}) representative of different orderings of the timescales, all
...
from an initial spin-frequency $\omega_0=10^{4}$ rad Hz.}
...
Plotted in figure~\ref{fig: NS A} are the spherical coordinates of the spin
vector in the effective body. This can be compared with the result
...
the moment spin-vector aligns with the effective body frame at which point the
precession ceases.}
...
50^{\circ}$. Moreover, we provide a new interpretation of this critical value
through the ratios of the loss of angular momentum and energy.

When $\tauP \ll \tauS$ we find that spin-vector aligns with the magnetic
...
with either the principal axis of the MOI tensor or the magnetic dipole.
However, we provide a new insight into this by showing that for all orderings
of the three timescales the spin-vector aligns with the principal axes of the
effective MOI tensor (defined in Sec.~\ref{sec: effective body frame}).  In this
sense, there is no persistent precession.

In conclusion, the anomalous torque is not important for realistic systems
