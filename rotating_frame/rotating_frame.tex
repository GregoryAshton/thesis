...
literature. This section considers solution exlusively in the rotating body frame
...
In the following sections we will develop our intution for the different types
...
\meta{Greg: we could potentially use more realistic values..}
...
initially at $a=50^{\circ}$ and $\epsA=5\times10^{-11}$. Evolving the modely
...
\meta{Greg: Ian I've forgotten what the issue was with this spherical solution
which led Goldrecih to his results?}
...
field $B_{S}$. This illustrates how the phase space is sliced into separate
regions by the ordering of the timescales.
\begin{equation}
B_{S} = \frac{2m}{R^{3}}
\end{equation}•
...
values; these correspond to either an oblate or prolate mass distribution.  In
all of the following we use only positive values of $\epsI$. It will be noted
when this is important, but therwise solutions are invariant to the sign of
$\epsI$. Finally we must choose the angle between the magnetic dipole and the
elastic deformations $\chi$, because of the symmetry of the star about the $\z$
...
vector having inclination angle of $50^{\circ}$ and azimuth $0^{\circ}$.
...
over equations~\eqref{eqn: eom} with this assumption yeilds
\begin{eqnarray}
...
geometric argument which provides a way to intepret this value as a natural
consequency of the conservation of energy and angular momentum.

For a biaxial mass distribution as described above, the conservation of energy
requies that
\begin{eqnarray}
1 & = & \frac{L_{x}^{2}}{2I_{x}E}+\frac{L_{y}^{2}}{2I_{y}E}+\frac{L_{z}^{2}}{2I_{z}E}
=\frac{L_{x}^{2}}{2I_{0}E}+\frac{L_{y}^{2}}{2I_{0}E}+\frac{L_{z}^{2}}{2I_{0}(1+\epsilon_{I})E},
\label{eqn: conservation of energy biaxial}
\end{eqnarray}
where $L_i$ are the components of the angular momentum. For the conservation of
...
$\sqrt{2I_{0}(1+\epsilon_{I})E}$ centered on the origin, while Eqn.~\eqref{eqn:
conservation of momentum biaxial} describes a sphere of radius $L$ also
centered on the origin. Physical solutions require the conservation of both
...
\end{figure}
\meta{Greg: should we have a figure of the cartesian space solutions of precession?}
...
angular momentum vector $L_i$ has a class of possible solution consisting of a
ring about the momentum-space $\z$.  Since $L_{i}=I_{ij}\omega^{j}$ and the
...
\item $\dot{A}<0$ The sphere shrinks with respect to the ellipsoid. In this
    case the intersection circles will increase in radius until $L\rightarrow
...
\dot{A}=\frac{1}{(2EI_{0})^{2}}
        \left(\frac{d}{dt}(L^{2})2EI_{0}-\frac{d}{dt}(2EI_{0})L^{2}\right),
\end{equation}
simplifying and writing in index notation such that $L^{2}=L^iL_i$, we have that
\begin{equation}
\dot{A}=\frac{1}{E^{2}}\left(2L^{i}\dot{L}_{i}E-\dot{E}L^{i}J_{i}\right).
\end{equation}
Now the energy is purely kinetic such that
$E=\frac{1}{2}I_{ij}\omega^{i}\omega^{j}$, and therefore $\dot{E}=T^{i}\omega_{i}$.
Then, noting that $\dot{L}^{i}=T^{i}$, we have
\begin{equation}
\dot{A}=\frac{1}{E^{2}}\left(L^{i}T_{i}I_{jk}\omega^{j}\omega^{k}
                             -T^{i}\omega_{i}L^{j}L_{j}\right).
\end{equation}
...
\end{eqnarray*}
Finally putting all the coefficeints back in, we have an expression for the averaged
...
such
\begin{eqnarray*}
L^{i}_{\;,t}+\epsilon^{ijk}\omega_{j}L_{k} & =
& N^{i}_{\textrm{spin down}}+
\epsA I_{0}\omega^{a}\hat{m}_{a}\epsilon^{ijk}\omega_{j}\hat{m}_{k}, \\
L^{i}_{\;,t}+
\epsilon^{ijk}\omega_{j}\left(L_{k}-\epsA I_{0}\omega^{a}\hat{m}_{a}\hat{m}_{k}\right)
& = & N^{i}_{\textrm{spin down}}, \\
L^{i}_{\;,t}+
\epsilon^{ijk}\omega_{j}\left(I_{ka}-\epsA I_{0}\hat{m}_{a}\hat{m}_{k}\right)\omega^{a}
& = & N^{i}_{\textrm{spin down}}.
\end{eqnarray*}•
By arranging the equation this way and making use of $L_{i}=I_{ij}\omega^{j}$
...
is the effective body frame with which the system aligns on the spin-down
timescale (much as it is the body frame with which the system aligns when the
anomalous torque contribution was neglected in Sec.~\ref{sec: biaxial NS
with no anomalous torque}).

For the three NS representive of different orderings of the timescales, all
three have a non-zero $\beta$. To familiarse the reader with the magnitudes in
the differeent regions in table \ref{tab: A B C params} we lists the relevant
...
are calcualted by numerical evolution of the Euler rigid body equations with the full
...
\begin{equation}
1 = \frac{L_{1}^{2}}{2\lambda_{-}E}+\frac{L_{2}^{2}}{2I_{0}E}+\frac{L_{3}^{2}}{2\lambda_{+}E}.
...
the conservation of energy still described a sphere of radius $L$. In order to
satify both conservation equations, the sphere and ellipsoid must intersect
bounding the radius of the sphere by
\begin{equation}
2E\lambda_{-}<L^{2}<2E\lambda_{+}.
\end{equation}

In the case of a biaxial moment of inertia we found that solution, where both
conservation laws are satified, corresponded to circles around the $\z$ axis.
...
\end{tabular}
\caption{Intersection of the sphere and ellipsoid defined in equations
    \eqref{eqn: conservation of energy triaxial} and  \eqref{eqn: conservation of mass triaxial} respectively. }
...
\begin{enumerate}[(a)]
\item When $2E\lambda_{-}<L^{2}<2EI_{0}$: in this region, curves form two sets of
    complete loops around the $\1$ axis. As $L^{2} \rightarrow 2E\lambda_{-}$
    the intersection loops will close up about the $\1$ axis.
\item When $L^{2} = 2EI_{0}$: this is a special case in which the intersection forms two
    closed ellipses
\item When $2EI_{0}>L^{2}>2E\lambda_{+}$: in this region, curves form two sets of
    complete loops around the $\3$ axis. As $L^{2} \rightarrow 2E\lambda_{+}$
...
of the sphere and ellipsoid, we can understand the cause of the unusual behavior
