...
with the aim being that in doing so, we can understand more about the physics
of neutron stars.
...
which apply to pulsars with periodic modulations. Such pulsars are particularly
...
investigated precession and its effects on the observed features of
...
the data.   They noted that measuring the spin-down and beam-shape
with any accuracy requires time averaging over periods~$\sim100$~days,
which smooths out any behaviour acting on this time-scale. Motivated by the
...
kindly supplied to us courtesy of Andrew Lyne \citep{Lyne2010}, and is reproduced in
Figure~\ref{fig: B1828-11 data}. From these observations,
the authors concluded that the individual measurements of $W_{10}$ for
...
of the precession or switching interpretations.  For this reason,
...
We note that there also exists high-resolution observations of the \emph{shape
parameter} of PSR~B1828-11 which are presented in \citet{Stairs2003} and
\citet{Lyne2012}. The shape parameter is defined to be the proportion of time
spent in one of two modes as determined from the integrated pulse profile.
These observations show that PSR~B1828-11 appears to `switch' between the modes on short
timescales (less than $\sim 8$~hrs). This, it seems, provides evidence for any
model where the switching occurs on short timescales, but is biased by a longer
timescale of $\sim 500$~days. The effect of precession on the shape parameter
has yet to be investigated, but it seems likely that it would be unable to
explain this short term variability. Nevertheless, this does not rule out
precession since there could independently exist short term magnetospheric
variability or a coupled hybrid model of precession biasing the switching (see
for example \citet{Jones2012}).  Moreover, for the switching model, this
observation also complicates the story since it requires stability over two
quite different timescales.  In this chapter, we will focus only on the long
timescale variations shown in Fig.~\ref{fig: B1828-11 data} and how these can
be interpreted under a deterministic switching model or precession; in effect
assuming that the short timescale variations are unrelated to the models under
investigation. In future, we would like to include these data sets and see how
...
introduction to this subject see \cite{sivia1996data, jaynes2003probability,
gelman2013bayesian}.
...
\end{equation}
The quantity $P(\data | \M_{i})$ is known as the \emph{marginal likelihood} of
...
$8$~ms, and all the variations about this mean are due to the noise. In the
following section we will develop models where at least some the variation is
explained by periodic modulations.
\begin{figure}[htb]
\centering
\includegraphics[width=0.75\textwidth]{Beamwidth_NoiseOnly_PosteriorFit}
...
There are multiple ways to parameterise the switching times in the model. For
the data analysis, we have chosen to parameterise by the total cycle duration
...
weighted sum of the two underlying spin-down rates as in segment C of the
...
when there is a misalignment between its spin axis and angular momentum vector. For this work
...
\end{equation}
This is Eqn.~\eqref{eqn: spin-down rate} derived in Section~\ref{sec: derivation of
...
$\mathbf{m}$ over a precessional cycle by a grey shaded region.  The region has
...
shows a single peak, often described as  core emission \mbox{\citep{Lyne1988}}.
Since we do not have a detailed model of the emission mechanism, we will now
consider the most rudimentary and natural beam geometry which fits this: a
circularly symmetric (about the beam-axis) intensity which falls-of with a
Gaussian function. In the following, we will repeat some of the work of
Sec.~\ref{sec: numerical beam-width} for completeness. Specifically, let us
define $\Delta d$ as the central angle between the observer's line-of-sight and
the beam (this is the spherical distance between the two points marked in
Figure~\ref{fig: delta theta delta phi}), then the intensity is
\begin{equation} \In(t) = \In_{0}\exp\left(-\frac{\Delta
d(t)^{2}}{2\rho^{2}}\right).  \label{eqn: beam intensity} \end{equation} Here
$\In_0$ is the intensity when observed directly along the dipole and $\rho$
measures the angular width of the beam.  From the spherical law of cosines, for
an observer located at $(\PhiO, \iota)$, we have \begin{align} \Delta d(t) &=
\cos^{-1}\left[\cos\Theta(t)\cos\iota +
\sin\Theta(t)\sin\iota\cos|\Delta\Phi(t)|\right].  \label{eqn: angular sep}
\end{align} The observer will see a maximum pulse intensity at $\Delta\Phi =
0$, given by \begin{align} \In_\textrm{max} = \In_{0} \exp\left(
-\frac{\left(\Theta(t) - \iota\right)^{2}} {2\rho^{2}}\right).  \label{eqn:
imax} \end{align}
...
the grounds that the observer would \emph{not} see tightly collimated pulsed
...
%\end{align}
%
%For PSR~B1828-11 we know (from the spin-down) that in this model $\chi \sim \pi/2$.
...
of freedom introduced by Eqn.~\eqref{eqn: rho2} is effectively removed. Instead,
we want to make it significantly larger than the (a priori unknown) posterior
...
posteriors and odds-ratios were robust to the choice, provided the
...
location. This corresponds to the grey region in Figure~\ref{fig: J observer
...
parameterised by $\rho_1 = \lambda \rho_2^0$ where $\lambda$ is an arbitrary
...
$2\theta\approx0.1$~rad over a precessional cycle. We have plotted a grey
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.75\textwidth]{{Beamwidth_ModifiedGaussian_0.1_RecreateGeometry}.png}
...
mean, and edges of the grey region in Figure~\ref{fig: modified-gaussian
beam-width recreate geometry}.
\begin{figure}[htb]
\centering
\includegraphics[width=0.75\textwidth]{{Beamwidth_ModifiedGaussian_0.1_RecreatePulse}.pdf}
...
growing from $1.44$ when $f=0.024$ to $3.21$ when $f=0.3$. In this region the
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.75\textwidth]{Beamwidth_Switching_Frac_Variation}
...
by the Parkes telescope, as discussed in \citet{Stairs2003} and \citet{Lyne2012}.
...
by including other data sources.
...
(ifor a discussion on this see Chapter~10 of \citet{gelman2013bayesian}). The MCMC
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.75\textwidth]{J_ObserverPlane_otherpole}
