...
$\dot{\nu}$. \meta{In this work, we will consider searches for gravitational
wave emission from
non-axisymmetric neutron stars at twice this frequency $f = 2\nu$; from hereon
all frequencies $f$ and spin-downs $\dot{f}$ refer to the pulsars CW emission.}
...
by \citet{Cordes1981}). \meta{The Crab ephemeris therefore provides an alternative
...
\meta{ The growth of mismatch as a power law is suggestive of random walk
timing noise models for which, as shown in Section~\ref{sec: TN interpretations
random walk models}, the root-mean-square phase residual also grows as a power law. The
power to which the phase residual grows depends on the type of random walk:
\citep{Boynton1972, Cordes1981} classified the Crab as undergoing
...
we directly calculate the mismatch due to a random walk and find that for
frequency-like noise, the mismatch scales as $\To^{3}$. This is an intriguiging
finding; it would be interesting to understand more about this relation.}
...
$\dot{\nu}$ and not the gravitational wave frequency $f$ used in the main body
of the chapter.}
