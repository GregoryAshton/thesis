...
were used in LIGO Einstein@Home all-sky search for periodic gravitational waves
...
Figure~\ref{fig: nu nudot}B: the pulsars with largest average glitch sizes have
...
same colour labels as in Figure~\ref{fig: Espinoza dF dF1}}
...
number of glitches during the search; this is also plotted in Figure~\ref{fig:
...
$\lambda_{E@H}$, as given on the right axis of Figure~\ref{fig: EAH_NdotgAve}A,
...
In Figure~\ref{fig: EAH_NdotgAve}B we plot this probability
...
that over timescales comparable to current and future searches (at least the
...
in depth later on in Section~\ref{sec: estimating the mismatch}.
...
in Figure~\ref{fig: varyingR}. First, we have error due to the assumption of small
...
In Figure~\ref{fig: follow-up} we plot the the semi-coherent mismatch for this
...
In Section~\ref{sec: mismatch due to glitches} we calculated the so-called
mismatch (defined in Section~\ref{sec: introduction to the mismatch}) for a simple
...
Figure~\ref{fig: extrapolation fit}.}
