...
remnant search we present parameters for the Cas A search only and note that
...
$\DnudotRglitch$ against the measured spin-down rate. The colored lines and shaded
...
for small numbers of segments. To investigate what happens when this is not the
case, and also to
...
three plots in Figure~\ref{fig: varying R} at point where the glitch deos not
...
componentsr. For each component $\Deltanu_i$ is recovered over an
...
In Section~\ref{sec: mismatch due to glitches} we calculated the metric
mismatch for a
...
These calculations assume that a single glitch occured during the search. To
...
The mismatch estimates potentially overestimate the effect by ignoring the
...
mismatch}, exhibit at least two distinct modes,
