...
capturing any additional nauances specific to this method. Specifically,
...
twice. Since the system is torque free the Euler angles evolutions are given
...
The intensity of radiation recieved by an observer will depend on the
...
governing ODEs to simulate this pulse intensity exactly This is deone in
...
unphysical simulation is intended to show the modulation of the individual
...
$\Phi=\PhiO$, then simplifiying we find that
\begin{align}
...
It is unlikely that the variatioins seen in Fig.~\ref{fig: intensity variation}
...
amount of dispersion from the interstellar medium. Therefore, pulsar astronmers
...
\PhiO$ increase beween $-\pi$ and $\pi$, and so the left hand side of the
...
constant; then the whole right hand side of ineqality~\eqref{eqn: 6737} is a
...
$2\pi - \Delta\Phi$. To convert this into the beam-width reporeted by pulsar
...
symmetry of the emmision geometry.
...
this here, but instead investigate a simple phenomlogical switching in which we
...
axis. In the presense of the anomalous torque this is done with
\begin{equation}
a_{0} = \beta(\epsI, \epsA, \chi),
\end{equation}

In Fig.~\ref{fig: no switching} we illustrate the behaviour of our simulation
in the absense of a switching event; the simulation parameters are listed in
...
zero. In addition to this precessional wobble, there is also an inaccuricies
...
simulation, with no switch occuring in the anomalous component.
That is we set
\begin{align}
...
the are marked by different colors.
...
which was interpreted by \citet{Lyne2010} as underdoing switching with a fractional
...
= 0.048 \textrm{ degs}
\end{align}
This is the new wobble angle which must occur everytimne B1828-11 undergoes a
