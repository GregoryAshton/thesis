...
of inertia as defined in Eqn.~\eqref{eqn: MOI} as given by
\begin{align}
...
\label{eqn: 6ODE z}
\end{align}
while the rearrange set of Euler angles ODEs from \citet{Landau1969} are given
by
\begin{align}
...
For the Euler angle equations, Eqns.~\eqref{eqn: 6ODE theta} to
...
Having defined the coupled ODEs in Eqns.~\eqref{eqn: 6ODE x} to Eqn.~\eqref{eqn:
...
on to simulate physical observables, however, let us first consider the
...
The solutions to Eqns.~\eqref{eqn: 6ODE x} to Eqn.~\eqref{eqn: 6ODE psi} were
studied by \citet{Jones2001}
...
\label{eqn: varphi TF}
\end{align}
This set of analytic solutions gives us a method to verify our numerical solver
...
prediction of Eqns.~\eqref{eqn: theta TF} to Eqn.~\eqref{eqn: varphi TF}. This
...
performing the subtraction in Eqn.~\eqref{eqn: 6ODE theta}. On short
timescales, these errors remain small; over sufficiently long timescale these
errors can accumulate anerror is eventually lead to a complete loss of
numerical accuracy. We must therefore be vigilant to ensure this does not occur
when considering realistic values.  \afterpage{\clearpage}
\begin{figure}[p]
    \centering
...
\caption{Solutions to the ODEs defined in Eqns.~\eqref{eqn: 6ODE x} to
...
as given in Eqns.~\eqref{eqn: theta TF} to Eqn.~\eqref{eqn: varphi TF}.}
...
Numerical solutions to Eqns.~\eqref{eqn: 6ODE x} to Eqn.~\eqref{eqn: 6ODE psi}
allow us to calculate the motion
...
Specifically, given a numerical solution to our system of coupled ODEs, i.e.
...
analytic prediction of Eqn.~\eqref{eqn: Jones 49} and Eqn.~\eqref{eqn: Jones
63}.}
\label{fig: PR 63}
\end{figure}
...
given in Eqns.~\eqref{eqn: 6ODE x} to Eqn.~\eqref{eqn: 6ODE psi}, contain both
...
the effects of precession on the pulsars observation features.
