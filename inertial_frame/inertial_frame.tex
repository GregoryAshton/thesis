...
a vector defined in the inertial frame, in the rotating frame the vector is
\begin{align}
...
\begin{equation}
|J_a| = |I^{b}_{\; a} \Omega_{b}|=\omega_{0}\sqrt{\sin^{2}a_{0} + \cos^{2}a_{0}(1+\epsI)^2}.
\end{equation}
...
The solutions to Eqn.~\eqref{eqn: 6ODE x} to Eqn.~\eqref{eqn: 6ODE psi} were
...
Solution to the rigid-body equations where derived in Eqn.~\eqref{eqn: free
precession x} to Eqn.~\eqref{eqn: free precession z}, transforming these into
the spherical components we have
\begin{align}
...
\end{align}
where $\n^a$ is the \emph{deformation axis}, a unit vector pointing along the
...
magnetic dipole. We will do this initially by studying the evolution of dipole
in the inertial frame via the the Euler angle rotations and then we will return
to this reference frame picture in Section~\ref{sec: understanding the motion
of m} to provide a deeper intuitive understanding.
...
similar periodic departures from their secular evolution's; in the same way as
for the phase residual these periodic modulations which can be amplified by the
