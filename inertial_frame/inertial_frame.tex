...
In Sec.~\ref{sec: rotating frame} we investigated the role of precession under
...
for the spin-down rate of a precessing pulsar derived in Sec.~\ref{sec:
derivation of the spin-down rate}; this derivation neglects the anomalous
torque discussed in Sec.~\ref{sec: rotating frame}, but we can be confident in
its predictions since we compare it against numerical solutions calculated with
the full torque in Sec.~\ref{sec: spin-down rate numerical B1828}.

This chapter is organised in the following way. In Sec.~\ref{sec: rotating} we
...
Sec.~\ref{sec: evolving the euler angles} we compare some typical numerical
...
intuition to understand the results, so in Sec.~\ref{sec: precessing pulsars}
...
in Sec.~\ref{sec: observable features: timing residuals}, Sec.~\ref{sec:
observable features: spin-down rate}, and Sec.~\ref{sec: observable features:
shape} for the timing residuals, spin-down rate, and shape of pulsations
respectively. Finally, in Sec.~\ref{sec: application switching and precession}
we give some preliminary findings on a hybrid model of precession and
magnetospheric switching before concluding in Sec.~\ref{sec: conclusion
inertial}.
...
are the same Euler angles as defined in Sec.~\ref{sec: euler rotation
matrices})
\begin{equation}
...
modulations which can be amplified by the EM torque. In Fig.~\ref{fig:
...
While Fig.~\ref{fig: spin-down rate normal} captures the essential features of
...
(introduced in Sec.~\ref{sec: understanding the motion of m}) is the the cone
...
Eqn.~\eqref{eqn: Theta 2}).  In the lower panel of Fig.~\ref{fig: spin-down
...
To quantify the changing shape of pulsations from B1828-11, astronomers
calculate the \emph{shape parameter} $S$. This is defined by first noting that
B1828-11 alternatives between two `modes', one narrow and one broad, then
...
Fig.~\ref{fig: polar angle variations} and found that the $\Theta$ has an
...
predictive analytic model for the beam-width of B1828-11.
...
(see Sec.~\ref{sec: derivation of the spin-down rate}) which will be used in
Chapter.~\ref{sec: testing model} and verified it against numerical solutions.
